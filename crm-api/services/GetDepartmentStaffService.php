...
class GetDepartmentStaffService extends Service
{
	/**
	 * 获取部门人员列表
	 ...
	 */
	public function getDepartmentStaff($city, $department)
	{
		if(!$city) {
		...
		$department_id = UserDepartment::find()
				->select(['id', 'name'])
				->andWhere('domain_id = :domain_id',['domain_id' => $domain_id['domain_id']])
