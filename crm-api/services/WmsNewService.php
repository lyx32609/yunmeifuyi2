...
class WmsNewService extends Service
{
    /**
     * 创建订单
     ...
     * @param $user_id 人员id
     *  */
    public function getCars($user_id, $is_cooperation, $company_category_id)
    {
        if(empty($user_id))
        {
        ...
        if($is_cooperation == 0){
            ...
            if($bind) {
                return $result = [
                    ...
                    [
                        ...
                        'car_driver_name' => $bind->user_name,
                        'car_driver_phone' => $bind->user_phone
                    ]
                    ],
                ]; 
            }
            ...
            $result = $http->request($url['url'], $url['public_key'] . $url['privace_key'], '', $url['protocol']); 
            if($result){
                return $result;
            } else {
                $this->setError('暂无车辆信息');
                return false;
            }
...
//         }
     ...
     *  
     *  */
    public function deliveryNoteNew($user_id, $is_cooperation, $company_category_id, $car_id, $car_name)
    {
        if(empty($user_id)  || empty($company_category_id) || empty($car_id) || empty($car_name))
        {
            $this->setError('参数不能为空');
            return false;
        }
        if($is_cooperation == 0){
            ...
            if($result){
                $car = BindCar::findOne(['user_id' => $user_id, 'car_id' => $car_id]);
                $car->status = 2;
                $car->save();
                return '成功';
            } else {
