...
class GetBusinessLocationByTypeService extends Service
{
/**
     * 员工定位查询
     ...
     * @return array
     */
    public function getLocationList($staffId,$start,$end,$offset,$limit,$type,$timeType)
    {
      if($timeType == '1'){
      ...
      if($timeType == '3'){
        ...
        $data = $this->locationList($staffId,$start,$end,$offset,$limit,$type);
        return $data;
    }
     ...
     * @return array
     */
    public function locationList($staffId,$start,$end,$offset,$limit,$type)
    {
        $people = User::find()
        ->andWhere('username = :user',[':user'=>$staffId])
        ->one();
        if(!$people)
        {
            $this->setError('员工不存在!');
          ...
          $result = $this->getNewLists($people['username'],$start,$end,$offset,$limit,$type);
          $list = array();
    ...
    private function getNewLists($user,$start,$end,$offset,$limit,$type)
    {
