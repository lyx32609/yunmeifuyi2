...
class GetCityDepartmentListService extends Service
{
	public function getCityDepartmentList($city, $department)
	{
		if(!$city){
		...
		if(!$department){
			...
			$department_id = UserDepartment::find()
					...
					->andWhere(['domain_id' => $domain_id['domain_id']])
					->andWhere(['is_show' => 1])
					->asArray()
					->one();
			if(!$department_id){
			...
			$result = User::find()
					->select(['id', 'username','password', 'name', 'phone', 'group_id', 'domain_id', 'department_id', 'rank'])
					->where(['domain_id' => $domain_id['domain_id']])
