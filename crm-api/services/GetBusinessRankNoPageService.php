...
use Yii;
class GetBusinessRankNoPageService extends Service
{
    /**
     *  获取员工签到记录
     * @param  [type] $area 省
    ...
    public function getBusinessRank($area,$city,$department,$stime,$etime,$type,$group)
    {
        ...
        {
            $result = $this->getGroupPersonRank($area,$city,$department,$group,$stime,$etime,$type);
        }
       
        if(!$result)
        {
            $this->setError('暂时没有数据');
            return false;
        }
         return $result;
    ...
    public function getGroupPersonRank($area,$city,$department,$group,$stime,$etime,$type)//获取分组或个人排名
    {
        ...
        if($group_data && (!$group))//有分组
        {
            ...
            {
                $sql = "select g.name,GROUP_CONCAT(u.username) as UserId from off_user as u left join off_user_group as g on g.id = u.group_id where u.group_id = '".$g_val['id']."'";
                $user_data = User::findBySql($sql)->asArray()->all(); 
                //return $user_data;
