...
$this->title = $model->id;
$this->params['breadcrumbs'][] = ['label' => '待审设备处理', 'url' => ['index']];
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="put-imei-view">
    <?php
    $data = PutImei::find()
        ->where(['user_id' => $model->user_id, 'status' => '1'])
        ->orderBy('id desc')
        ->asArray()
        ->all();
    $user = User::find()
    ...
    <table class="table table-striped table-bordered">
        ...
        foreach ($data as $k => $v) {
            ...
            <tr data-key="<?php echo $v['id'] ?>">
                ...
                <td>
                    <a class="btn btn-sm btn-info" href="/imei-deal/deal?id=<?php echo $v['id'] ?>">同意</a>
                </td>
            </tr>
            <?php
        }
