...
<div class="petition-create">
    ...
    <form action="/petition/create" id="signForm" enctype="multipart/form-data" method="post">
        ...
        <div class="cont">
            <div class="cont_tit" style="width:100%;"><h5>用证签呈</h5></div>
            <input type="hidden" name="type" value="6">
            <input name="_csrf-backend" type="hidden" id="_csrf" value="<?= Yii::$app->request->csrfToken?>">
            <dl>
                <dd><label for="title"><span>*</span>证件名称：</label><input id="title" type="text" name="message[name]"></dd>
                <dd><label for="name2"><span>*</span>用途：</label><textarea id="name2" name="message[purpose]"></textarea></dd>
                <dd>
                    <label for="title"><span>*</span>证件类型：</label>
                    <span id="isCheckbox">
                        <input type="checkbox" name="message[flag][0]" value="0"><label for="">原件</label>
                        <input type="checkbox" name="message[flag][1]" value="1"><label for="">复印件</label>
                        <input type="checkbox" name="message[flag][2]" value="2"><label for="">扫描件</label>
                    </span>
                </dd>
    ...
    $(function(){
        $("#signForm").validate({
            ...
            submitHandler:function(form){
                if($("#isCheckbox input").is(":checked")){
                    form.submit();   //提交表单
                }else{
                    alert("请选择证件类型");
                }
