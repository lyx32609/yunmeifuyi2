            ...
            buttonText: function (options) {
                ...
                if (options.length == 0) {
                    ...
                    options.each(function () {
                        ...
                        selectAll.push($(this).text());
                    });

                    //console.log(selectAll);
                    return selectAll.join(",");
                }
...
<div class="user-form">
    ...
    $include_department_id = $model->include_department_id;
//    $include_department_id = '';

    $user_department_id = explode(',',$include_department_id);
//    var_dump($arr);die;
//    var_dump($user_department_id);die;
    ...
    <?php } else { ?>
        ...
        <span style="color: red">(*-*新增部门时需要将新增部门和原有部门都勾选上*-*）</span>
        <select name="User[department][]" id="department_id" multiple="multiple">
            <?php
            $through = Yii::$app->params['through'];
            $identy_user_id = Yii::$app->user->identity->id;
            $identy_user_companyid = Yii::$app->user->identity->company_categroy_id;
            //获取登录人所在公司的部门列表
            if ((in_array($identy_user_id, $through)) || ($rank == 30)) {
                $departments = UserDepartment::find()
                    ->select(['id', 'name'])
//                    ->where(['company' => $identy_user_companyid])
                    ->asArray()
                    ->all();
            }else{
                $departments = UserDepartment::find()
...
//            var_dump($user_department_id);//die;
            foreach ($departments as $key => $vo) {
                /*判断是否所属部门为空*/
                if($include_department_id){
                    if(in_array($vo['id'],$user_department_id)){
