...
<script>
    $(function () {
        getall();
        <?php  if(!$model->area){?>
        $('.field-userbusinessnotessearch-city').css('display', 'none');
        <?php }?>
        <?php  if(!$model->city){?>
        $('.field-userbusinessnotessearch-company_id').css('display', 'none');
        <?php }?>
    ...
    function getCity() {
        var domainid = $('#userbusinessnotessearch-area').val() ? $('#userbusinessnotessearch-area').val() : 0;
        $.ajax({
            type: "GET",
            url: "/user-sign/get-city?id=" + domainid + "&pid=<?php echo $_GET['id'] ?? ''; ?>",
            async: false,
            success: function (data) {
                $("select#userbusinessnotessearch-city").html(data);
                $('.field-userbusinessnotessearch-city').css('display', 'block');
            }
        });
    }
    ...
    //获取公司
    function getCompany() {
        var area_id = $('#userbusinessnotessearch-area').val() ? $('#userbusinessnotessearch-area').val() : 0;
        var city_id = $('#userbusinessnotessearch-city').val() ? $('#userbusinessnotessearch-city').val() : 0;
        $.ajax({
            type: "GET",
            ...
            success: function (data) {
                console.log(data);
                $("select#userbusinessnotessearch-company_id").html(data);
                $('.field-userbusinessnotessearch-company_id').css('display', 'block');
            }
        });
    }
    ...
    function getDepartment() {
        var area_id = $('#userbusinessnotessearch-area').val() ? $('#userbusinessnotessearch-area').val() : 0;
        var city_id = $('#userbusinessnotessearch-city').val() ? $('#userbusinessnotessearch-city').val() : 0;
        var company_id = $('#userbusinessnotessearch-company_id').val() ? $('#userbusinessnotessearch-company_id').val() : 0;
        $.ajax({
            type: "GET",
            ...
            success: function (data) {
                console.log(data);
                $("select#userbusinessnotessearch-department").html(data);
            }
        });
    }
</script>
